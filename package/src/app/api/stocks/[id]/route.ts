...
export async function PATCH(request: Request, context: RouteParams) {
  ...
  try {
    ...
    console.error('Error al actualizar stock en Baserow', error);
    const message =
      error instanceof Error
        ? error.message
        : 'Error desconocido al actualizar el stock';
...
export async function DELETE(_request: Request, context: RouteParams) {
  ...
  try {
    await deleteTableRow(STOCK_TABLE_ID, rowId);
    ...
    console.error('Error al eliminar stock en Baserow', error);
    const message =
      error instanceof Error
        ? error.message
        : 'Error desconocido al eliminar el stock';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
