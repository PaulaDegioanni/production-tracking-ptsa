...
export async function PATCH(request: Request, context: RouteParams) {
  ...
  try {
    ...
    console.error('Error al actualizar camión en Baserow', error);
    const message =
      error instanceof Error
        ? error.message
        : 'Error desconocido al actualizar el camión';
...
export async function DELETE(_request: Request, context: RouteParams) {
  ...
  try {
    await deleteTableRow(TRUCKS_TABLE_ID, rowId);
    ...
    console.error('Error al eliminar camión en Baserow', error);
    const message =
      error instanceof Error
        ? error.message
        : 'Error desconocido al eliminar el camión';
    return NextResponse.json({ error: message }, { status: 500 });
  }
}
